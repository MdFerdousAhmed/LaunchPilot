...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrors({ api: err.message || 'Login failed. Please verify your credentials.' });
    } finally {
      setLoading(false);
    }
  };

  // Demo Login Handler with auto-fill & auto-submit fallback signup
  const handleDemoLogin = async () => {
    setErrors({});
    setLoading(true);
    // Auto-fill values on UI
    setEmail('demo@launchpilot.app');
    setPassword('password123');

    try {
      // Attempt login
      await login('demo@launchpilot.app', 'password123');
    } catch (err: any) {
      try {
        // Fallback signup if the database doesn't have the user yet
        await signup('Demo Founder', 'demo@launchpilot.app', 'password123');
      } catch (signupErr: any) {
        setErrors({ api: signupErr.message || 'Demo login failed.' });
      }
    } finally {
      setLoading(false);
    }
  ...
  // Google Login Handler (Simulated OAuth flow via fallback signin)
      ...
      <div className="w-full max-w-md space-y-8 backdrop-blur-xl bg-zinc-900/40 p-8 rounded-2xl border border-zinc-800/80 shadow-2xl relative z-10">
        ...
        <div className="space-y-2.5">
          ...
          >
            ...
            <span>Fast Entry (Demo Sandbox)</span>
          </button>
        </div>
      </div>
    </div>
